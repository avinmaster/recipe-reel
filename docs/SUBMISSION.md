...
```
Not a Gemini wrapper. RecipeReel's accuracy comes from FUSING two independent signals a transcript-only tool can't: Whisper gives the ordered, timestamped steps; Qwen2.5-VL vision + on-screen OCR gives the exact quantities — which cooking videos show on screen and never say aloud. A provenance layer tags every field spoken / on-screen / inferred, and leaves unknown quantities null instead of hallucinating. That fusion is genuine GPU work, not one API call.

Real AMD compute. The heavy perception (Whisper-large-v3 + Qwen2.5-VL-7B) runs on the AMD Instinct MI300X via ROCm. The repo ships notebooks/amd_recipereel_demo.ipynb — it runs both models on the GPU and prints the live device string + peak GPU memory, so the AMD usage is reproducible, not just asserted. ROCm PyTorch reports as CUDA, so the identical code runs on a laptop, in CI, and on the MI300X; GET /api/v1/meta reports the live device / GPU / ROCm version.

Real Gemma, one code path. Gemma does the structured reasoning over an OpenAI-compatible endpoint: free Google AI Studio (real Gemma-4), Fireworks AI, or self-hosted vLLM on the MI300X (AMD-hosted Gemma — targets the Best AMD-Hosted Gemma prize). Only base_url / model / key change.

Instant to evaluate. The live URL (https://infra.tailc95f92.ts.net/) is always-on in MOCK_MODE — run the full pipeline with no keys or setup. Locally: cp .env.example .env && make dev, or one docker run. Tests: 24 passed.

Scales beyond the hackathon. Perception is embarrassingly parallel per video and every request is independent, so throughput scales by batching videos across the MI300X's 192 GB and adding GPU workers behind the async queue — no rewrite. The schema.org/Recipe JSON-LD output drops into recipe sites, meal-planners, and grocery-cart integrations — the wedge from a consumer tool to a video-understanding API creators and food platforms pay for.
```
